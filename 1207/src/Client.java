	...
	private void run() throws IOException {
		...
		while (true) {
			...
			if (line.startsWith("SUBMITNAME")) {
				...
				//temp.setProblem(a1); // 같이 온 문제번호를 찾아서 문제 Label부분에다가 덮어씌우기	
				temp.stopwatch(); // timer 작동
			} else if (line.startsWith("NATK")) {
				//temp.setProblem(a1); // 같이 온 문제번호를 찾아서 문제 Lbel부분에다가 덮어씌우기				
